...
function OrdersPanel(props: OrdersPanelProps) {
    ...
    const [filteredOrders, filteredMatchedOrders] = useMemo(() => {
        const matchedOrders = safeLocalStorageGetItem(
            `order-details-${accountId}`
        )
        const parsedMatchedOrders: LocalOrder[] =
            matchedOrders ? JSON.parse(matchedOrders) : []
        ...
        const filteredOrders = Object.values(orders).filter(
            ({ amount, orderId }) =>
                amount > 0 &&
                !filteredMatchedOrders.some(({ id }) => id === orderId)
        )
        ...
        panelBody = (
            ...
            >
                {Object.values(globalOrders).map((counterpartyOrder) => {
                    const title =
                        orders[counterpartyOrder.id] ?
                            `${
                                orders[counterpartyOrder.id].side === "buy" ?
                                    "Buy"
                                :   "Sell"
                                ...
                                KATANA_ADDRESS_TO_TICKER[
                                    `0x${
                                        orders[counterpartyOrder.id].baseToken
                                            .address
                                    }`
                                ]
                            } ${
                                ...
                                KATANA_ADDRESS_TO_TICKER[
                                    `0x${
                                        orders[counterpartyOrder.id].quoteToken
                                            .address
                                    }`
                                ]
                            }`
                    ...
                    const status =
                        counterpartyOrder.id in orders ? "ACTIVE"
                        : (
                            counterpartyOrder.state === "Cancelled" &&
                            !(counterpartyOrder.id in orders) &&
                            savedOrders.some(
                                ({ id }) => id === counterpartyOrder.id
                            )
                        ) ?
                            "MATCHED"
                        : counterpartyOrder.state === "Cancelled" ? "VERIFIED"
                        : counterpartyOrder.state.toUpperCase()

                    const ago = dayjs
                        .unix(counterpartyOrder.timestamp)
                        .fromNow()

                    const textColor =
                        status === "ACTIVE" || status === "MATCHED" ? "green"
