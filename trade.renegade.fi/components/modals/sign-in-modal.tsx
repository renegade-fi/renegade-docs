...
export function SignInModal({ onClose }: { onClose: () => void }) {
  return (
    <Modal isCentered isOpen onClose={onClose} size="sm">
      ...
      />
      <ModalContent height="228" background="surfaces.1" borderRadius="10">
        <ModalHeader paddingBottom="0">Unlock your Wallet</ModalHeader>
        <ModalCloseButton />
        <Fade transition={{ enter: { duration: 0.25 } }} in>
...
export function Content({ onClose }: { onClose: () => void }) {
  ...
  // useEffect(() => {
  //   if (client.chain?.id && signMessage) {
  //     signMessage({
  //       message: `${ROOT_KEY_MESSAGE_PREFIX} ${client.chain?.id}`,
  //     })
  //   }
  // }, [client.chain?.id, signMessage])
      ...
      <Flex justifyContent="center" flexDirection="column" gap="4">
        ...
        <Flex gap="2">
          <Checkbox
            ...
            value={rememberMe}
          />
          <Tooltip placement="right" label={REMEMBER_ME_TOOLTIP}>
            <Flex gap="1">
              <Text color="white.60" fontSize="0.9em">
                Remember Me
              </Text>
