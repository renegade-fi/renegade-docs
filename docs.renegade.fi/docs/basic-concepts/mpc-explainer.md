...
compute a function on that data, and sends the function output to all the
parties.
...
to understand, where MPC calculations are done on [Shamir Secret
Shares](https://en.wikipedia.org/wiki/Shamir%27s_Secret_Sharing) of the
individual private inputs. Understanding how MPC works on the protocol level is
out-of-scope for these docs, but one of the easier-to-understand secret-shared
MPC protocols is BGW: [See
...
Architecture]](/basic-concepts/mpc-zkp), we will see how we combine the idea of
